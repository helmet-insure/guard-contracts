...
contract Guard is ERC20UpgradeSafe, Configurable {

	function initialize(address governor_, address locked, address farm, address eco) public initializer {
		Governable.initialize(governor_);
		ERC20UpgradeSafe.__ERC20_init("Guard.insure Governance Token", "Guard");
		...
		_mint(eco,      1250_0000 * 10 ** uint256(decimals));       // 12.5%
	}

}


contract GuardTimelock is TokenTimelockUpgradeSafe {

    function __GuardTimelock_init(IERC20 token, address beneficiary, uint256 releaseTime) public initializer {
        __TokenTimelock_init_unchained(token, beneficiary, releaseTime);
    }
